...
#define $guard$
    ...
    void Struct::Parent(const std::string& parentName)
    {
        parents.push_back(parentName);
    }

    void Struct::Field(const std::string& field)
    {
        this->field = field;
    }

    void Struct::PrintHeader(google::protobuf::io::Printer& printer) const
    {
        printer.Print(R"(struct $name$ )",
            "name", name);

        if (!parents.empty())
            printer.Print(": $parents$ ", "parents", Parents());
    ...
    void Struct::PrintSource(google::protobuf::io::Printer& printer, const std::string& scope) const
    {
        Entities::PrintSource(printer, scope + name + "::");
    }

    std::string Struct::Parents() const
    {
        std::string res;
        ForEach(
            parents, [&res](std::string_view parent)
    ...
    void Function::Parameter(const std::string& parameter, const std::string& init)
    {
        parameters.push_back({parameter, init});
    }

    void Function::PrintHeader(google::protobuf::io::Printer& printer) const
    {
        printer.Print("$static$$virtual$$result$ $name$($parameters$)$const$$override$$abstract$;\n", 
            "result", result, "name", name, "parameters", HeaderParameters(), "const", (flags & fConst) != 0 ? " const" : "", 
            "virtual", (flags & fVirtual) != 0 ? "virtual " : "", 
            "abstract", (flags & fAbstract) != 0 ? " = 0" : "", 
            "override", (flags & fOverride) != 0 ? " override" : "",
            "static", (flags & fStatic) != 0 ? "static " : "");
    }

    void Function::PrintSource(google::protobuf::io::Printer& printer, const std::string& scope) const
    {
        printer.Print("$result$ $scope$$name$($parameters$)$const$ ", "result", result, "scope", scope, "name", name, "parameters", SourceParameters(), "const", (flags & fConst) != 0 ? " const" : "");
    ...
    std::string Function::HeaderParameters() const
    {
        std::string res;
        ForEach(
            parameters, [&res](std::pair<std::string_view, std::string_view> parameter)
            {
                res += parameter.first;

                if (!parameter.second.empty()) {
                    res += " = ";
                    res += parameter.second;
                }
            },
            [&res]()
            {
    ...
    std::string Function::SourceParameters() const
    {
        std::string res;
        ForEach(
            parameters, [&res](std::pair<std::string_view, std::string_view> parameter)
    ...
    {
        printer.Print("$explicit$$name$($parameters$)$default$$delete$;\n", 
            "explicit", (flags & cExplicit) ? "explicit " : "",
    ...
    {
        return !( (flags&cDefault) || (flags&cDelete));
    }
    ...
    SourceSnippet::SourceSnippet(const std::string& snippet)
        : Entity(false, true)
        , snippet(snippet)
    {}

    void SourceSnippet::PrintHeader(google::protobuf::io::Printer& printer) const
    {}

    void SourceSnippet::PrintSource(google::protobuf::io::Printer& printer, const std::string& scope) const
    {
    ...
    StructForwardDeclaration::StructForwardDeclaration(const std::string& name)
        : Entity(true, false)
        , name(name)
    {}

    void StructForwardDeclaration::PrintHeader(google::protobuf::io::Printer& printer) const
    {
        printer.Print("struct $name$;\n", "name", name);
    }

    void StructForwardDeclaration::PrintSource(google::protobuf::io::Printer& printer, const std::string& scope) const
    {}
    ...
    EnumDeclaration::EnumDeclaration(const std::string& name, const std::vector<std::pair<std::string, int>>& members, bool scoped)
        : Entity(true, false)
        , name(name)
        , members(members)
        , scoped(scoped)
    ...
    {
        printer.Print("enum$scoped$ $name$ {\n", "name", name, "scoped", scoped ?  " class" : "");
        printer.Indent();
